...
eval "$("$PYTHON_CMD" "$CONFIG_QUERY" models-shell)"
...
echo -e "${CYN}==========================================================${RST}"
echo ""
echo -e "  This will download and configure AI models onto"
echo -e "  your USB drive. You'll get to CHOOSE which models"
echo -e "  to install from a curated list."
echo ""
echo -e "   - Multiple preset models (uncensored + standard)"
echo -e "   - Custom model support (bring your own GGUF)"
echo -e "   - Minimum USB space: 8 GB (16 GB recommended)"
echo ""
echo -e "  Make sure you have a good internet connection!"
echo ""
read -n 1 -s -r -p "  Press any key to start the setup..."
echo -e "\n"
...
# ================================================================
if [ ${#MODEL_NUMS[@]} -eq 0 ]; then
    echo -e "${RED}ERROR: Model catalog is empty!${RST}"
    echo -e "${RED}Check that Shared/config/models.json exists and is valid.${RST}"
    ...
    for TOKEN in "${TOKENS[@]}"; do
        ...
        if [ "$T" = "c" ] || [ "$T" = "custom" ]; then
            ...
            if $VALID; then
                ...
                echo -e "${RED}  Invalid number '$T' - skipping (valid: ${MODEL_NUMS[0]}-${MODEL_NUMS[-1]})${RST}"
            fi
        else
