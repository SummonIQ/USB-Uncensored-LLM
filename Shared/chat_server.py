...
def _get_hw_stats():
    ...
    if plat == "Darwin":
        ...
        try:
            ...
            size = ctypes.c_size_t(ctypes.sizeof(total_ram))
            libc.sysctlbyname(b"hw.memsize", ctypes.byref(total_ram), ctypes.byref(size), None, 0)
            
            # Get used RAM (approximation via vm_stat)
            # On Mac, accurate used RAM without psutil is tricky, so we'll use a shell command
            import subprocess
            vm = subprocess.check_output(["vm_stat"]).decode()
            vm_lines = vm.split('\n')
            pages_free = int(vm_lines[1].split()[-1].strip('.'))
            pages_speculative = int(vm_lines[6].split()[-1].strip('.'))
            page_size = 4096 # Standard for macOS
            free_ram = (pages_free + pages_speculative) * page_size
            ram = round((1.0 - (free_ram / total_ram.value)) * 100.0, 1)
        except Exception:
            ram = 0.0

        # CPU via top (simple one-shot)
        try:
            import subprocess
            cmd = "top -l 1 -n 0 | grep 'CPU usage'"
            output = subprocess.check_output(cmd, shell=True).decode()
            # "CPU usage: 10.55% user, 8.25% sys, 81.19% idle"
            idle = float(output.split('idle')[0].split(',')[-1].strip().strip('%'))
